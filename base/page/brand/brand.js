jQuery1_11_2(function () {
  ...
  const encodedString_brand = locationSplit[1];
  const decodedString_brand = decodeURIComponent(encodedString_brand);
  //alert(decodedString)
  /*메뉴 컬러 분기*/
  ...
  $(".board-list[data-cate]").each(function () {
    if ($(this).data("cate") === decodedString_brand) {
  ...
  if (decodedString_brand === "brandstory") {
    $(".pageTitle").find(">h2").text("brand story").css("color", "#f0468c");
  }
  if (decodedString_brand === "doongs") {
    $(".pageTitle").find(">h2").text("doongs").css("color", "#f0468c");
  }
});
